...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const scenes = [
    ...
    document.getElementById('scene-blackout'),
  ...
  btnStart.addEventListener('click', (e) => {
    startBgMusic();
    // Prime the custom audio on user gesture so mobile browsers allow playback in Scene 5
    if (customAudio) {
      customAudio.load();
    }
  ...
  function choreographScene1() {
    const l1 = document.getElementById('line-1-1');
    const l2 = document.getElementById('line-1-2');
    const l3 = document.getElementById('line-1-3');
    const btn = document.getElementById('btn-scene-1');

    setTimeout(() => l1.classList.add('revealed'), 400);
    setTimeout(() => l2.classList.add('revealed'), 1600);
    setTimeout(() => l3.classList.add('revealed'), 2800);
    setTimeout(() => {
  ...
  // --- SCENE 2: The Realization (College & Ordinary Days) ---
  function choreographScene2() {
    const lines = [
      document.getElementById('line-2-1'),
      document.getElementById('line-2-2'),
      document.getElementById('line-2-3'),
      document.getElementById('line-2-4'),
      document.getElementById('line-2-5'),
      document.getElementById('line-2-6')
    ];
    const btn = document.getElementById('btn-scene-2');

    const delays = [400, 1600, 2900, 4400, 5600, 6900];
    lines.forEach((line, i) => {
      setTimeout(() => line.classList.add('revealed'), delays[i]);
    });
  ...
  // --- SCENE 3: The Vulnerable Honesty ---
  function choreographScene3() {
    const lines = [
      document.getElementById('line-3-1'),
      document.getElementById('line-3-2'),
      document.getElementById('line-3-3'),
      document.getElementById('line-3-4'),
      document.getElementById('line-3-5'),
      document.getElementById('line-3-6'),
      document.getElementById('line-3-7')
    ];
    const btn = document.getElementById('btn-scene-3');

    const delays = [400, 1600, 2800, 4200, 5800, 7000, 8200];
    lines.forEach((line, i) => {
      setTimeout(() => line.classList.add('revealed'), delays[i]);
    });
  ...
  // --- SCENE 4: The Playful Break (Mini-Game) ---
  function choreographScene4() {
    // Buttons are ready for her interaction
  }

  const choiceButtons = document.querySelectorAll('.choice-btn');
  const gameReply1 = document.getElementById('game-reply-1');
  const gameReply2 = document.getElementById('game-reply-2');
  const btnScene4 = document.getElementById('btn-scene-4');
  let gameAnswered = false;

  const responses = {
    smile: "Interesting choice. Your smile definitely lights up the room.",
    eyes: "Interesting choice. There's a quiet calm in your eyes.",
    hair: "Interesting choice. Especially when you tuck it away.",
    mood: "Interesting choice. You'd be surprised how much warmth you carry."
  };
  ...
  choiceButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      starfield.burst(rect.left + rect.width / 2, rect.top + rect.height / 2, 18);
      soundEngine.playChime(3);

      gameReply1.textContent = responses[choice] || "Interesting choice.";
      gameReply1.classList.add('revealed');

      setTimeout(() => {
        gameReply2.textContent = "Actually… I notice when you're happy.";
        gameReply2.classList.add('revealed');
        starfield.triggerShootingStar();

        // Softly highlight her happy smile photo in the aesthetic background
        const photo1 = document.querySelector('.c-photo-1');
        if (photo1) {
          photo1.style.opacity = '0.62';
          photo1.style.transform = 'translateY(-18px) rotate(-2deg) scale(1.06)';
          photo1.style.boxShadow = '0 0 50px rgba(244, 114, 182, 0.7)';
          setTimeout(() => {
  ...
  btnScene4.addEventListener('click', () => goToScene(5, btnScene4));

  // --- SCENE 5: The Rhythm & The Song ---
  ...
  function choreographScene5() {
    ...
    setTimeout(() => heartText.classList.add('revealed'), 600);
    setTimeout(() => heartSubtext.classList.add('revealed'), 1600);

    // Fade in music card and play Afreen motif after 3.2s
    setTimeout(() => {
      musicCard.style.opacity = '1';
      musicCard.style.transform = 'translateY(0)';
      musicMemoryNote.classList.add('revealed');

      startPlayingAfreen();
    }, 3200);

    setTimeout(() => {
      btnScene5.style.display = 'inline-flex';
      btnScene5.style.opacity = '0';
      btnScene5.style.transition = 'opacity 0.8s ease';
      requestAnimationFrame(() => btnScene5.style.opacity = '1');
    }, 6500);
  }

  customAudio.addEventListener('ended', () => {
    musicDisk.classList.remove('spinning');
    soundwaveBars.classList.remove('music-playing');
    resumeBgMusic();
  });

  function startPlayingAfreen() {
    pauseBgMusic();
    musicDisk.classList.add('spinning');
    soundwaveBars.classList.add('music-playing');
  ...
  musicDisk.addEventListener('click', toggleAfreenMusic);
  ...
  // --- SCENE 6: The Blackout Climax (Improvised Ending) ---
  function choreographScene6() {
    ...
    const bh1 = document.getElementById('bo-heart-1');
    const bh2 = document.getElementById('bo-heart-2');
    const bh3 = document.getElementById('bo-heart-3');
    ...
    setTimeout(() => bo1.classList.add('revealed'), 2200);
    setTimeout(() => bo2.classList.add('revealed'), 3800);
    setTimeout(() => bo3.classList.add('revealed'), 5400);

    // Heartbeat phrases with single realistic thud each
    setTimeout(() => {
      soundEngine.playSingleHeartbeat();
      bh1.classList.add('revealed');
    }, 7200);

    setTimeout(() => {
      soundEngine.playSingleHeartbeat();
      bh2.classList.add('revealed');
    }, 9400);

    setTimeout(() => {
      soundEngine.playSingleHeartbeat();
      bh3.classList.add('revealed');
    }, 11600);

    // Blackout resolution
    setTimeout(() => bo4.classList.add('revealed'), 13800);
    setTimeout(() => bo5.classList.add('revealed'), 15400);
    setTimeout(() => bo6.classList.add('revealed'), 17000);

    // Stars return softly, ambient music restores softly
    setTimeout(() => {
      starfield.setDimmed(false, 0.02);
      soundEngine.fadeAmbient(0.28, 3.0);
      resumeBgMusic();
      boConc.style.opacity = '1';
    }, 18800);
  }

  const btnGoNow = document.getElementById('btn-go-now');
  btnGoNow.addEventListener('click', () => {
    goToScene(7, btnGoNow);
  });

  // --- SCENE 7: The "Wait." Twist & Closure ---
  function choreographScene7() {
    const waitText = document.getElementById('wait-text');
    const takeCare = document.getElementById('take-care-text');
    const flashBox = document.getElementById('wait-flash-box');
    const sigContent = document.getElementById('signature-content');

    // "Wait." 😄
    setTimeout(() => {
      takeCare.style.opacity = '1';
    }, 1200);

    // Transition into quiet signature canopy
    setTimeout(() => {
      flashBox.style.opacity = '0';
      flashBox.style.transition = 'opacity 0.8s ease';
      setTimeout(() => {
        flashBox.style.display = 'none';
        sigContent.style.opacity = '1';
        sigContent.style.transform = 'translateY(0)';
      }, 800);
    }, 3000);
  }
